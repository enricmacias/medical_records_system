...
4. Review structured data in a clear web UI (read-only by default) and **edit** pet and owner fields when needed (clinical summary is extraction-generated and read-only in v1). When the LLM summary fails, the UI indicates a **rule-based fallback** via `meta.clinical_summary_source`. **See which fields were missing or uncertain** via visual highlights driven by `meta.missing_fields` and `meta.extraction_confidence`. UI available in **English or Spanish** (site toggle); independent of document language.
