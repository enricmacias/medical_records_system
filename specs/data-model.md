...
|---|---|---|---|
| `pet.name` | Name | Nombre | Animal name only — never a compound header string or generic document word. **Validated on extraction** (proper-name filter); invalid candidates dropped; **ranked heuristics** pick the best plausible name (see Extraction notes). **Value not translated** by site language. Manual PATCH accepts any non-empty string. |
...
1. **Heuristics first** (`adapters/text_hints.py`): normalize text; detect language; parse Spanish/English labels; chip/clinic/address; split dated visit blocks; diagnosis and medication keyword hints; **inline compound demographic lines** and **label-free species/breed header patterns** (below). Demographic heuristics scan the **header region (first ~100 lines)**. **Global inference fallbacks** (`infer_*_from_text`) fill missing pet/owner demographic fields by scanning the header sample for labeled and pipe-table patterns (same strategy as `infer_species_from_text`); they also run **`validate_and_refine_pet_name`** so an already-set `pet.name` can be upgraded when a higher-ranked candidate exists. **Pet name extraction** (see below) collects multiple header candidates, **scores** them, and picks the **highest-scoring validated** name — not the first match in document order. **Breed inference** still uses first-valid catalog match (document order); see item 3.
   - **Candidate sources** (non-exhaustive): colon labels (`Pet:` / `Mascota:` / `Paciente:` / `Patient:`; `Nombre:` / `Name:`); quoted labels (`Pet: "Buddy"`); label + word (`Paciente MARLEY`, `Pet Buddy`); `Nombre` / `Name` prefix lines; quoted phrases (`Se llama 'Luna'`, `Known as "Max"`); standalone quoted lines; standalone **ALL-CAPS** lines/tokens; **title-case** standalone lines when on the same or adjacent line as species/breed/sex/chip context.
   - **Scoring** (higher wins among validated candidates): stronger weight for pet-context colon labels than for `Nombre:`/`Name:`; bonuses for demographic proximity on the same/adjacent line and for repeated mentions in the header; penalties for weak sources (standalone caps/title-case/quoted line) that match an **owner** first token or appear on owner-labeled lines. Cross-check uses `Owner:` / `Propietario:` lines and **`Nombre PET OWNER`** splits (below).
   - **Format / case:** mixed-case values accepted on colon labels and `Nombre`/`Name` prefix lines; surrounding `"` / `'` stripped from inferred values; title-case standalone lines are **ignored** without nearby demographic context.
   - **Validation:** `validated_pet_name()` rejects generic non-name words (e.g. Summary, Grammar, punctuation), species/sex tokens, and other header noise; invalid candidates are dropped. **`validate_and_refine_pet_name`** runs at the end of `build_layout_hints` and `apply_global_demographic_inference` and is the **final authority** for `pet.name` in hints (may override earlier inline or label assignments).
2. **Inline compound demographics** (header region):
   ...
   - **`Nombre PET OWNER…`** (mixed-case supported) → `pet.name` (first token) and `owner.name` (remainder), e.g. `Nombre Luna Beatriz Abarca`
   ...
   - Compound-name repair when generic `Nombre`/`Name` captures the full line into `pet.name` (may be superseded by ranked refinement at end of heuristic pass)
