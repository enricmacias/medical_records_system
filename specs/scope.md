...
- Structured extraction via a **hybrid pipeline**:
  - layout/visit **heuristics** (Spanish/English clinic headers — including **inline compound lines** such as `ALYA - Nacimiento: DATE` and `Hembra Estado: …` (sex only); **label-free species/breed lines** such as `CANINA - YORKSHIRE TERRIER` or standalone `Canino` without `Especie:` / `Raza:` labels; **global demographic inference** over the first ~100 header lines and pipe-table rows; **pet name** and **breed validation** during extraction; species **`Dog`/`Cat`** and sex **`Male`/`Female`** normalization; dated historial blocks; diagnosis/med hints)
