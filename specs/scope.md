...
- Structured extraction via a **hybrid pipeline**:
  - layout/visit **heuristics** (Spanish/English clinic headers — including **inline compound lines** such as `ALYA - Nacimiento: DATE` and `Hembra Estado: …` (sex only); **`Nombre PET OWNER…`** splits; **label-free species/breed lines** such as `CANINA - YORKSHIRE TERRIER` or standalone `Canino` without `Especie:` / `Raza:` labels; **global demographic inference** over the first ~100 header lines and pipe-table rows; **ranked pet-name heuristics** with proper-name validation and format/case patterns; **breed catalog validation** during extraction; species **`Dog`/`Cat`** and sex **`Male`/`Female`** normalization; dated historial blocks; diagnosis/med hints)
  - optional **Ollama** structured outputs (`qwen2.5:7b` by default) for demographics and a **text-first** clinical summary (`ClinicalSummaryOutput`) per `LLM_CLINICAL_MODE`; `meta.clinical_summary_source` records LLM vs heuristic provenance
  - **FakeLLM** adapter for tests and demos without Ollama (heuristic clinical summary; `clinical_summary_source=heuristic`)
- Persist original file, raw text, and structured JSON (SQLite + filesystem)
- React UI: upload, list, on-demand extracted-text preview, structured record (read-only by default; edit mode for **Pet** and **Owner** only; **Clinical summary** and **Meta** always read-only), **processing-state polling** with **progressive section loading** (pet/owner/meta as soon as ready) and **percent/step progress feedback** for the clinical summary, **fallback notice** when LLM summary fails (`meta.clinical_summary_source=heuristic_fallback`), **site language toggle (English / Español)** with localized UI labels and date display, **confidence UX** (highlight missing and low-confidence fields in the structured form)
